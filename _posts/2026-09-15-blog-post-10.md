...
p(x | μ, Σ) = (1/((2π)^(d/2)|Σ|^(1/2))) exp(-1/2(x-μ)ᵀΣ⁻¹(x-μ))
...
μ_MLE = (1/n)Σxᵢ (the sample mean)

Σ_MLE = (1/n)Σ(xᵢ - μ_MLE)(xᵢ - μ_MLE)ᵀ (the sample covariance)
...
p(x | θ) = Σₖ πₖ 𝓝(x | μₖ, Σₖ)

Each component k contributes a Gaussian 𝓝(x | μₖ, Σₖ) weighted by mixing coefficient πₖ. The parameters θ encompass all mixing coefficients, means, and covariances.
...
The prior probability p(z_k = 1) = πₖ reflects how common component k is overall. The conditional probability p(x | z_k = 1) = 𝓝(x | μₖ, Σₖ) describes data from component k.
...
log p(D | θ) = Σᵢ log[Σₖ πₖ 𝓝(xᵢ | μₖ, Σₖ)]

The logarithm of a sum has no simple derivative. We cannot solve ∂/∂θ log p(D | θ) = 0 analytically. The coupling between parameters across the sum blocks direct optimization.
...
γᵢₖ = p(z_k = 1 | xᵢ, θ) = [πₖ 𝓝(xᵢ | μₖ, Σₖ)] / [Σⱼ πⱼ 𝓝(xᵢ | μⱼ, Σⱼ)]
...
The lower bound equals the true log-likelihood when q(z) matches the posterior distribution p(z|x,θ). The E-step sets q to this optimal distribution. The M-step maximizes the bound over θ while keeping q fixed.

### The E-Step: Computing Responsibilities

Given current parameters θ⁽ᵗ⁾, we compute responsibilities using Bayes' rule:

γᵢₖ⁽ᵗ⁾ = [πₖ⁽ᵗ⁾ 𝓝(xᵢ | μₖ⁽ᵗ⁾, Σₖ⁽ᵗ⁾)] / [Σⱼ πⱼ⁽ᵗ⁾ 𝓝(xᵢ | μⱼ⁽ᵗ⁾, Σⱼ⁽ᵗ⁾)]
